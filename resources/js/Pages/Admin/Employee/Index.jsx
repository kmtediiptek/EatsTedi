...
export default function Index({ total_employees, ...props }) {
    ...
    const onUpdate = (e) => {
        e.preventDefault();
        router.post(
            `/admin/setting/employee/${userName}`,
            {
                _method: "put",
                ...data,
                status: data.status.name,
    ...
    const onDelete = (userName) => {
        destroy(route("admin.employee.destroy", userName), {
            onSuccess: () => {
                toast.success("Employee has been deleted!"), setIsToast(false);
            },
        });
    };
            ...
            <Container>
                ...
                {employees.length > 0 && (
                    <div className="flex w-full justify-between items-center">
                        ...
                        <p className="text-sm text-third mt-10">
                            Total employees:{" "}
                            <span className="font-bold">{total_employees}</span>{" "}
                        </p>
                    </div>
                )}
                {/* End employees */}
                ...
                >
                    <div className="flex justify-end gap-2 justify-center">
                        ...
                        <PrimaryButton onClick={() => onDelete(userName)}>
                            Yes
                        </PrimaryButton>
                    </div>
                </Toast>
