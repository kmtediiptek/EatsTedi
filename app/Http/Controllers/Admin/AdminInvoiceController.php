...
class AdminInvoiceController extends Controller
{
    ...
    public function store(AdminTableInvoiceRequest $request)
    {
        ...
        if ($request->id) {
            $invoice = Invoice::select('payment_id')->where('user_id', Auth::user()->id)->where('order_id', $request->order_id)->first();
        }
